...
class AddressBook {
    ...
    // Get the number of contacts in a particular city
    getCountByCity(city) {
        const count = this.contacts.filter(contact => contact.city.toLowerCase() === city.toLowerCase()).length;
        return count > 0 ? `There are ${count} contact(s) in ${city}.` : `No contacts found in ${city}.`;
    }

    // Get the number of contacts in a particular state
    getCountByState(state) {
        const count = this.contacts.filter(contact => contact.state.toLowerCase() === state.toLowerCase()).length;
        return count > 0 ? `There are ${count} contact(s) in ${state}.` : `No contacts found in ${state}.`;
    }
...
// Get count by city
console.log("\nGetting contact count for Bhopal...");
console.log(myAddressBook.getCountByCity("Bhopal"));

// Get count by state
console.log("\nGetting contact count for Madhya Pradesh...");
console.log(myAddressBook.getCountByState("Madhya Pradesh"));
